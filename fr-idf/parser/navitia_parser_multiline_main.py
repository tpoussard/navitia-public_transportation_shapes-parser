...
with open('../ntfs/geometries.txt', 'r') as file:
    ...
    # look up for mistakes
            ...
            if properties['line_id'] == '':  # check if lineID is empty
                notFoundIndex += 1
        #  format shape data from string format to json format
            #  remove parenthesis
    ...
    # in order to be able to add them manually to mapbox for instance
            if index % 110 == 0:
                ...
                formerIndex = index + 1 # increment former index to write in the file name next time
                if notFoundIndex != 0:  # check if there was missing shape
                    print('\nMissing lineIds =', notFoundIndex)
                arrayOfGeojsonItems = [] # empty list that has already been saved
                notFoundIndex = 0
        index += 1
